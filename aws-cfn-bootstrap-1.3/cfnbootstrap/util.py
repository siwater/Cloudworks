...
def get_instance_identity_document():
    resp = requests.get(adaptor.Adaptor.instance_identity_document_url())
    resp.raise_for_status()
    return resp.text.rstrip()

@retry_on_failure(max_tries=10)
def get_instance_identity_signature():
    resp = requests.get(adaptor.Adaptor.instance_identity_signature_url())
    resp.raise_for_status()
    return resp.text.rstrip()
...
def _fetch_instance_id():
    resp = requests.get(adaptor.Adaptor.instance_id_url(), timeout=2)
    resp.raise_for_status()
...
def get_role_creds(name):
    resp = requests.get(adaptor.Adaptor.iam_security_credentials_url(name))
    resp.raise_for_status()
